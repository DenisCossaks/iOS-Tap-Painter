...
- (HSVType)hsvType {
    return RGB_to_HSV([self rgbType]);
}
...
{
    if([self alpha] != 1.0) {
        NSString* string = [NSString stringWithFormat:@"0x%0.8X", (unsigned int)self.rgbaHex];
//        NSLog(@"hexString: %@ from Color: %@", string, self);
        return string;
    }
    else {
        NSString* string = [NSString stringWithFormat:@"0x%0.6X", (unsigned int)self.rgbHex];
//        NSLog(@"hexString: %@ from Color: %@", string, self);
        return string;
    }
}
...
+ (UIColor *)colorWithString:(NSString *)stringToConvert {
	...
	switch (i) {
		case 2: // monochrome
			...
			break;
		case 4: // RGB
            NSLog(@"Color components: %.2f %.2f %.2f", c[0], c[1], c[2]);
...
+ (UIColor *)colorWithHexString:(NSString *)stringToConvert 
{
    ...
    if([stringToConvert length] == noAlphsLength)
